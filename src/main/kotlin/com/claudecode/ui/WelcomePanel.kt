...
class WelcomePanel(
    ...
    /**
     * Returns "(v1.0.6)" using the version baked in at build time. We
     * don't call PluginManagerCore here — it's @ApiStatus.Internal in
     * 2026.2+ EAP and the marketplace verifier flags it. The BuildInfo
     * constant is generated by Gradle from project.version on every
     * compile, so it always matches the artifact. The "-dev.<timestamp>"
     * portion is stripped so dev builds display the clean release-style
     * version too.
     */
    private fun currentPluginVersionSuffix(): String {
        val version = com.claudecode.BuildInfo.VERSION
            .substringBefore("-dev.")
            .ifBlank { return "" }
        return "(v$version)"
    }
